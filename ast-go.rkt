...
(module+ test
  ...
  (run-suites
   (list
    ...
    (test-suite "macro"
                (check-equal? (emit-macro (go:macro 'test null (list (quote (quote (+ 1 1))))))
                              ""))

    (test-suite "quote"
                (check-equal? (emit-quote (go:quote (quote (+ 1 1))))
                              "(+ 1 1)"))
    ...
    (test-suite "primitive"
                (test-case "nil"
                  (check-equal? (ast->go (go:expr 'nil))
                                "nil"))
                (test-case "bool"
                  (check-equal? (ast->go (go:expr #t)) "true")
                  (check-equal? (ast->go (go:expr #f)) "false"))
                (test-case "number"
                  (check-equal? (ast->go (go:expr 666))    "666")
                  (check-equal? (ast->go (go:expr 666.6))  "666.6")
                  (check-equal? (ast->go (go:expr -666))   "-666")
                  (check-equal? (ast->go (go:expr -666.6)) "-666.6"))
                (test-case "string"
                  (check-equal? (ast->go (go:expr "hello")) "\"hello\""))
                (test-case "identifier"
                  (check-equal? (ast->go (go:expr 'runtime.GOMAXPROCS))
                                "runtime.GOMAXPROCS")))
    (test-suite "dummy"
                (check-equal?
                 (emit-expr
    ...
    (test-suite "complex"
                (test-case "cli"
                  (check-equal?
                   (ast->go (list (go:expr (go:package 'main))
